    ...
    def saml_login(self, session=None):
        ...
        if 'sso' in request.args:
            ...
            if len(errors) == 0:
                ...
                username = auth.get_nameid()
                email = auth.get_nameid()
                log.info("Saml assertion is successfull")
                ...
                if not user:
                    log.info("User is not registered. Automatically registering as normal user")
                    user = models.User(
                        username=username,
                        email=email,
                        superuser=False)
        ...
        log.info("hit lower section")
        log.info(str(paint_logout))
        log.info(str(request.args.get('state')))
        if not paint_logout:
